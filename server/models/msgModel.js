...
async function createMessage(username, email, message) {
  return query(
    "INSERT INTO public.valleyview_messages (username, email, message) VALUES ($1, $2, $3) RETURNING *",
...
const getAllMessages = async () => {
  const result = await query(
    "SELECT id, username, email, message, created_at FROM public.valleyview_messages ORDER BY id DESC"
  );
  return result;
};
...
const deleteMessage = async (messageId) => {
  const result = await query(
    "DELETE FROM public.valleyview_messages WHERE id = $1 RETURNING * ",
    [messageId]
  );
  return result;
};
